...
const verifyNumber = async () => {
  const unformattedPhoneNumber = formData.phoneNumber.replace(/[-() ]/g, ''); // unformat the phone number before it passes
  try {
...
const formattedFormData = {
    phoneNumber: formData.phoneNumber.replace(/[-() ]/g, ''), // remove formatting
    availability: formatAvailabilityForBackend(formData.availability),
    monthly_narcan: formData.monthly_narcan
};
...
try {
    ...
    const verifyResponse = await fetch(`https://prevent-overdose-github-io.onrender.com/api/narcan/${formattedFormData.phoneNumber}`);
    const verifyResult = await verifyResponse.json();

    if (!verifyResult.exists) {
        toast.error("Phone number not found. Please register your organization first.");
        return;
    }

    // send the form data to update availability in the backend
    const response = await fetch("https://prevent-overdose-github-io.onrender.com/api/narcan/updateAvailability", {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
